...
describe('Forward Requests', () => {
  it('forwards a chat message', async () => {
    ...
    forward = await forwardMessage(BOT_TOKEN, { chat_id: PRIVATE_CHAT_ID, from_chat_id: PRIVATE_CHAT_ID, message_id: send.message_id })
  ...
  it('forwards multiple chat messages', async () => {
    ...
    forward = await forwardMessages(BOT_TOKEN, { chat_id: PRIVATE_CHAT_ID, from_chat_id: PRIVATE_CHAT_ID, message_ids: [s1.message_id, s2.message_id] })
    if (forward instanceof Error) throw forward
