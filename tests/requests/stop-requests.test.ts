...
describe('Stop Requests', () => {
  it('stops a live location', async () => {
    ...
    location = await sendLocation({ chat_id: PRIVATE_CHAT_ID, latitude: 0, live_period: 60, longitude: 0 })
    if (location instanceof Error) throw location

    stop = await stopMessageLiveLocation({ chat_id: PRIVATE_CHAT_ID, message_id: location.message_id })

    if (stop instanceof Error) {
      await deleteMessage({ chat_id: PRIVATE_CHAT_ID, message_id: location.message_id })
      throw stop
    }
  ...
  it('stops a poll', async () => {
    ...
    stop = await stopPoll({ chat_id: PRIVATE_CHAT_ID, message_id: poll.message_id })

    if (stop instanceof Error) {
      await deleteMessage({ chat_id: PRIVATE_CHAT_ID, message_id: poll.message_id })
      throw stop
    }
