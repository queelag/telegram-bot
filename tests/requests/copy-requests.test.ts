...
describe('Copy Requests', () => {
  it('copies a chat message', async () => {
    ...
    copy = await copyMessage({ chat_id: PRIVATE_CHAT_ID, from_chat_id: PRIVATE_CHAT_ID, message_id: send.message_id })
  ...
  it('copies multiple chat messages', async () => {
    ...
    copy = await copyMessages({ chat_id: PRIVATE_CHAT_ID, from_chat_id: PRIVATE_CHAT_ID, message_ids: [s1.message_id, s2.message_id] })
    if (copy instanceof Error) throw copy
